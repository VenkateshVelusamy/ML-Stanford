...
% y(k) - we need to recode the labels as vectors containing only values 0 or 1 (
% as mentioned in page 5 of ex4.mlx)
y_vect = zeros(num_labels, m); % 10*5000
for i=1:m
  y_vect(y(i),i)=1;
end

J = (1/m) * sum ( sum ( (-y_vect) .* log(h_theta) - (1-y_vect) .* log(1-h_theta) ));

% We should not regularize the terms that correspond to the bias. 
...
for t=1:m
    ...
    % Step 1
	a1 = X(t,:); % X already have a bias Line 43 (1*401)
    a1 = a1'; % (401*1)
